...
% required input dimension, common matrix dimensions between variables, and
% NaN, inf, and real checking.  All errors report messages to the screen. 
% Dimension failures caues the stop_flag to be set.  NaN, inf, and real
% failues cause a warning message only. 
%  
% Input: estruct - error checking structure 
%   estruct.func_name = 'string';              function name 
%   estruct.variable(i).name = 'string';       i-th variable name from
%   input estruct.variable(i).req_dim = nxm array;   i-th input required
%   dimensions (opt.) estruct.variable(i).var = jxk;             i-th
%   variable estruct.variable(i).type = 'type_string';  i-th variable type
%   (optional) 
% 
%   The required dimensions (req_dim) is nxm where multiple options are
%   allowed. For example, a valid input could be 1x3 or 1x4 which would
%   lead to the 
%     req_dim = [1 3; 1 4]; 
%   For matching inputs between multiple variables, use a number greater
%   than 900 to indicate that the dimensions must match.  For example if
%   variable #1 and variable #2 must match in the row dimension, but have
%   different requirements the column dimension, it would be handled as
%   follows 
%     estruct.variable(1).req_dim = [901 2]; estruct.variable(2).req_dim =
%     [901 3; 901 4]; 
%   This input means that the variables #1 and #2 must match in the row
%   dimension and the column dimensions are 2 and 3 or 4, respectively.   
%   There is no restriction on the number of variables, the number of
%   variables that must match a given dimension, or the number of allowed
%   dimensions in the required dimension (req_dim) field. 
% 
%   The 'type' of variable input is used for the sanity checking of the
%   values of that variable.  Following are the current valid types and
%   thier bounds. 
%      LLA_RAD  
...
%      KEPLER_ORBIT 
%          Keplerian orbit check, input [a e], (1x2) or (nx2) a is in
%          meters, e is dimensionless.  Checks perigee and apogee to be  
%          above the surface of the Earth.  If neither apsis is above the
%          surface of the Earth a warning message is issued. 
%      STRING  
%          variable must be a string  
% 
%   If an invalid or unknown type is specified a warning message will be
%   issued. The type checking is completely optional. 
% 
...
%       not be set and execution will continute.  Error messages will be
%       printed to the screen. 
 
% Written by: Jimmy LaMance 4/15/98 Copyright (c) 1998 by Constell, Inc.
% 
...
% if the DEBUG_MODE has not been set, set it to 0, the default error
% checking mode 
...
% Initialize the variable for similar dimension comparison between
% variables 
sim_compare = []; 
var_compare = []; 
stop_flag = 0; 
...
for i = 1:num_to_check 
  ...
  if isfield(estruct.variable(i),'req_dim') 
    if ~isempty(estruct.variable(i).req_dim)  
      ...
      % Get the input and required dimension in new variable names for ease
      % of use 
      estruct.variable(i).in_dim = size(estruct.variable(i).var); 
      in_dim = estruct.variable(i).in_dim; 
      req_dim = estruct.variable(i).req_dim; 
     
      % Make sure that the input required dimension and input variable
      % dimensions 
      % match in array sizes (?x2) 
      if size(req_dim,2) == size(in_dim,2) 
     
        % Loop over the required dimension to see if there are any matches.
        % A valid match at this point includes a match in a single
        % dimension if the other dimensions are variable (900+). 
        for jjj = 1:size(req_dim,1)                     
          ...
          % Check for a single dimension match with a 900 in the other
          % dimension 
      ...
      if valid_dim == 0 
       fprintf('%s input %s dimension is incorrect. \n',... 
        ...
        for k = 1:size(estruct.variable(i).req_dim,1) 
          ...
          % See if any of the valid dimensions have > 900 and substitute
          % with n 
          I_n = find(valid_dim_num > 900); 
       
          % Replace the 900 with a single digit 9.  If the 900 stays then 
          % the conversion to a string ends up with 3 digits.  This is the
          % simple way to avoid that.  The 9 is just a placeholder at this
          % point. 
          if ~isempty(I_n) 
          ...
          % Fill in the variable length array dimensions with the letter
          % 'n' ASCII 110 
          if ~isempty(I_n) 
      ...
      % Now mark this variable if it has to be size compared to another
      % variable 
